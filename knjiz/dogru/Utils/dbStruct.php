...
class dbStructUpdater
{
	...
	*/
	...
	function compareTablesStruct($struct2, $struct1)
	{
		$destTabNames = $this->getTableList($struct2);
		$sourceTabNames = $this->getTableList($struct1);

		$common = array_intersect($destTabNames, $sourceTabNames);
		sort($common);
		$sql = array();
		$patern = '/\) ENGINE\=([a-z]+).*DEFAULT CHARSET\=([a-z0-9]+)(.*COLLATE\=[a-z_0-9]+)*/i';
		foreach ($common as $tab)
		{
			$destSql = $this->getTabSql($struct2, $tab, true);
			$sourceSql = $this->getTabSql($struct1, $tab, true);
			$m1 = $m2 = array();
			if (preg_match($patern, $destSql, $m1) && preg_match($patern, $sourceSql, $m2))
			{
				//Check Engine
				if($m1[1] != $m2[1] || $m1[2] != $m2[2] || $m1[3] != $m2[3])
					$sql[] = "ALTER TABLE `$tab` ENGINE=$m2[1] DEFAULT CHARSET=$m2[2]$m2[3]";
			}
		}
		return $sql;
	}
