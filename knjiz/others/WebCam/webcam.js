...
window.webcam = {
	...
	swf_url: 'pravi/others/WebCam/webcam.swf', // URI to webcam.swf movie (defaults to cwd)
	...
	freeze: function() {
		...
		$('BUTTON.btn-take-in-div').button('option', 'disabled', true);
		$('BUTTON.btn-save-in-div').button('option', 'disabled', false);
		$('BUTTON.btn-reset-in-div').button('option', 'disabled', false);
	},

	upload: function(url, callback) {
		$('BUTTON.btn-take-in-div').button('option', 'disabled', true);
		$('BUTTON.btn-save-in-div').button('option', 'disabled', true);
		$('BUTTON.btn-reset-in-div').button('option', 'disabled', true);
		...
		var mov = this.get_movie();
		mov._upload( this.api_url );
	},

	reset: function() {
		// reset movie after taking snapshot
		this.get_movie()._reset();
		$('BUTTON.btn-take-in-div').button('option', 'disabled', false);
		$('BUTTON.btn-save-in-div').button('option', 'disabled', true);
		$('BUTTON.btn-reset-in-div').button('option', 'disabled', true);
	},
...
//	alert(response);
	...
	var obj = UploadTypeObj[UploadTypeImage];
	obj.UploadComplete(response[0],response[1],response[2]);
	uploadOK.dialog("close");
//	$(this).dialog("close");
}
function do_upload() {
	webcam.upload(null,uploadCB);
	uploadOK = $(this);
//	SingleImage.UploadComplete();
}
function do_freeze() {
	webcam.freeze();
}
...
function CloseFrame(r)
{
	var par = window.parent;
	var div = par.document.getElementById('DIV_TakePhoto');
	if (r=='r') {par.window.location.reload(); return};
