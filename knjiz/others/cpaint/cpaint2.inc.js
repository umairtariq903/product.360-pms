...
function cpaint() {
  ...
  */
   ...
   *
   */
  this.autoCorrectEncoding = false;
  ...
  this.call = function() {
    ...
    if (use_stack != -1) {
      ...
      if (config['proxy_url'] != '') {
        ...
        __cpaint_stack[use_stack].call_direct(arguments, this.autoCorrectEncoding);
      }
...
function cpaint_call() {
  ...
  * @return   void
  */
  this.call_direct = function(call_arguments, autoCorrectEncoding) {
	...
	if (autoCorrectEncoding)
		querystring += "&cpaint_autocorrect=1";
	else
		querystring += "&cpaint_autocorrect=0";
    ...
    if (config['use_cpaint_api'] == true) {
      ...
      for (i = 3; i < call_arguments.length; i++) {
        ...
        if ((typeof call_arguments[i] == 'string'
          ...
          && isFinite(call_arguments[i]) && false) {
          // numerical value, convert it first
          json = JSON2.stringify(Number(call_arguments[i]));

        } else {
          json = JSON2.stringify(call_arguments[i]);
        }
		json = String.CleanMsWordChars(json, true);
		querystring += '&cpaint_argument[]=' + encodeURIComponent(json);
      }
    ...
    for (i = 3; i < call_arguments.length; i++) {
      ...
      if (config['use_cpaint_api'] == true) {
        ...
        if ((typeof call_arguments[i] == 'string'
          ...
          // numerical value, convert it first
          querystring += encodeURIComponent(querystring_argument_prefix + JSON2.stringify(Number(call_arguments[i])) + '&');

        } else {
          querystring += encodeURIComponent(querystring_argument_prefix + JSON2.stringify(call_arguments[i]) + '&');
        }
  ...
  */
  var callback = function() {
    ...
    if (httpobj.readyState == 4
      ...
      if (response != null) {
		AjaxResponseWaiting = false;
		if (typeof client_callback == 'function')
			client_callback(response, httpobj.responseText);
		else if (typeof client_callback == 'object' && client_callback._callback)
