...
* @version    $Id$
 *
 NOT: Orijinal cpaint2.inc.php değiştirilmiştir [DOGRU]
*/
...
//---- classes -----------------------------------------------------------------
  ...
  */
  class cpaint {
    ...
    * @return   void
    */
    function start($input_encoding = 'UTF-8', $setReturnValue = false) {
      $user_function  = '';
      $arguments      = array();
...
//          } // end: if
		  ...
		  $arguments[$key] = json_decode($value);
		  $page = PageController::$_CurrentInstance;
		  $convertQuots = true;
		  if($page)
			  $convertQuots = $page->ConvertQuots;
		  if (isset($_REQUEST['cpaint_autocorrect']) && $_REQUEST['cpaint_autocorrect'] == "1")
		  	  Kodlama::KarakterKodlamaDuzelt ($arguments[$key], true, $convertQuots);
        } // end: foreach

        if($_REQUEST['cpaint_autocorrect'] != "1")
			$arguments = cpaint_transformer::decode_array($arguments, $this->basenode->get_encoding());

        if (isset($this->api_functions[$user_function]) && is_array($this->api_functions[$user_function])
          && is_callable($this->api_functions[$user_function]['call'])) {
          // a valid API function is to be called
		  try
		  {
			Transaction::Begin();
            $returnVal = call_user_func_array($this->api_functions[$user_function]['call'], $arguments);
			if (is_object($returnVal) || is_array($returnVal))
				$returnVal = Kodlama::JSON($returnVal);
			Transaction::Commit();
		  }catch(Exception $ext){
			  Transaction::Rollback();
			  $returnVal = $ext->getMessage();
		  }
		  if ($setReturnValue)
			$this->set_data($returnVal);

        } else if ($user_function != '') {
          // desired function is not registered as API function
		  $defaultErrMessage = "[CPAINT] '$user_function' was passed that is not allowed to execute on this server.";
		  if (function_exists('cpaintErrorMessage'))
			  $defaultErrMessage = cpaintErrorMessage($user_function, $defaultErrMessage);
          $this->basenode->set_data($defaultErrMessage);
        }
      } // end: if
    }
  ...
  */
  class cpaint_transformer {
    ...
    * @return   string
    */
    static function encode($data, $encoding) {
      // convert string
    ...
    * @return   array
    */
    static function decode_array($data, $encoding) {
      $return_value = array();
    ...
    * @return   string
    */
    static function find_output_charset($encoding) {
      $return_value = 'UTF-8';
